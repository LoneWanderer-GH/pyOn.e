...
| 10 | Bonding BLE SMP | OS transparent (Android/iOS) | `bleak.pair()` rejeté par module → auth AES seule | ⚠️ Dépend firmware | 🔍 À vérifier |
...
### Check #2 — Vérification bonding BLE
**Test** : observer si `read_gatt_char(FBDE0104)` retourne `NotAuthorized` ou `NotPermitted`.  
**Si oui** : investiguer bonding BlueZ via `bluetoothctl` ou `hcitool`.
